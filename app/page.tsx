...
import {
  LayoutDashboard, Calendar, Settings, X, AlertTriangle, Search, Trash2, Split, Users, Filter, MapPin, Plus, Minus, Database, Download, Upload, Save, LogOut, Printer, Shield, CheckCircle
} from 'lucide-react';
...
import { cleanupInvalidUserData, forceLogoutIfInvalid } from '../lib/cleanup';
    ...
    <div className="flex flex-col bg-white shrink-0 shadow-sm z-40 print-header" style={{ fontFamily: '"Comic Sans MS", cursive, sans-serif' }}>
      ...
      <div className="flex flex-wrap items-center justify-between px-4 py-2 bg-slate-50 border-b border-slate-200 gap-2 w-full">
        ...
        <div className="flex items-center gap-1 no-export">
          ...
          <button onClick={() => handleSaveToDatabase()} className={`flex items-center justify-center bg-green-500 hover:bg-green-600 text-white border border-green-600 rounded p-2 shadow-sm transition-all font-bold text-sm no-print ${!RoleManager.hasPermission(currentUser, 'save_to_database') ? 'hidden' : ''}`} title="Sauvegarder en base de données">
            <Save size={16} />
          </button>

          {RoleManager.hasPermission(currentUser, 'save_to_database') && isSaving && (
          ...
          {RoleManager.hasPermission(currentUser, 'manage_users') && (
            <>
              <button onClick={() => {
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'manage' | 'planning' | 'config' | 'data' | 'users' | 'security'>('planning');
  ...
  useEffect(() => {
    ...
    // Nettoyer les données invalides au démarrage
    ...
    if (savedUser) {
      try {
        ...
        // Valider que l'utilisateur a un email @supnum.mr
  ...
  // Validation périodique de l'utilisateur
  useEffect(() => {
    if (!isClient || !currentUser) return;

    const interval = setInterval(() => {
      forceLogoutIfInvalid(setCurrentUser);
    }, 30000); // Vérifier toutes les 30 secondes

    return () => clearInterval(interval);
  }, [currentUser, isClient, setCurrentUser]);

  // Sauvegarde persistante de l'utilisateur
  useEffect(() => {
    if (isClient) {
      if (currentUser) {
        // Valider l'utilisateur avant de le sauvegarder
        if (currentUser.email && currentUser.email.endsWith('@supnum.mr')) {
          localStorage.setItem('supnum_user', JSON.stringify(currentUser));
        } else {
          console.warn('Tentative de sauvegarder un utilisateur invalide:', currentUser);
          setCurrentUser(null);
        }
      } else {
        localStorage.removeItem('supnum_user');
      ...
      <div id="export-container" className="flex flex-1 flex-col overflow-hidden bg-white">
        ...
        <div className="flex flex-1 overflow-hidden">
          <aside className="w-12 bg-slate-900 text-slate-400 flex flex-col items-center py-4 gap-6 shrink-0 z-30 no-export">
            ...
            {RoleManager.hasPermission(currentUser, 'manage_users') && (
              <>
                ...
                {/* Security - Admin only */}
                {(() => {
                  const hasPermission = RoleManager.hasPermission(currentUser, 'manage_users');
                  console.log('Security button - Current user:', currentUser);
                  console.log('Security button - Has permission:', hasPermission);
                  return hasPermission;
                })() && (
                  <button 
                    onClick={() => setActiveTab('security')} 
                    className={`p-2 rounded-xl transition-colors ${activeTab === 'security' ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-800'}`}
                    title="Sécurité"
                  >
                    <Shield size={20} />
                  </button>
          ...
          <main className="flex-1 flex flex-col min-w-0 h-full">
                  ...
                  <table className="w-full text-xs text-left border-collapse table-fixed">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {assignmentRows.filter(r =>
                        r.semester === semester &&
                        (activeMainGroup === "Tous" || r.mainGroup === activeMainGroup) &&
                        (
            ...
            {activeTab === 'users' && RoleManager.hasPermission(currentUser, 'manage_users') && (
              <div className="p-6 overflow-auto h-full bg-slate-50">
                <UserManagement />
              </div>
            )}
            ...
            {activeTab === 'security' && RoleManager.hasPermission(currentUser, 'manage_users') && (
              <div className="p-6 overflow-auto h-full bg-slate-50">
                <SecurityContent />
              </div>
            )}
...
function SecurityContent() {
  const [activeTab, setActiveTab] = React.useState<'overview' | 'policies' | 'users' | 'setup'>('overview');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="border-b border-slate-200 mb-8">
          <nav className="flex space-x-8">
            {[
              { id: 'overview', label: 'Aperçu', icon: Search },
              { id: 'policies', label: 'Politiques RLS', icon: Database },
              { id: 'users', label: 'Utilisateurs', icon: Users },
              { id: 'setup', label: 'Installation', icon: Settings }
            ].map(tab => (
              <button
                key={tab.id}
